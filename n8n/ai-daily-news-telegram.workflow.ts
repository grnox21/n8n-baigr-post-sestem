...
const splitForTelegram = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: 'javaScript',
      jsCode: `// ╔══════════════════════════════════════════════════════════════════╗
// ║  مين بتوصلو النشرة — زيد أو شيل رقم من هون وبس            ║
// ╠══════════════════════════════════════════════════════════════════╣
// ║  كل رقم بين علامتين، وفاصلة بعدكل واحد.                 ║
// ║  محادثة شخص = رقم عادي متل '1038608008'                  ║
// ║  قناة = رقم سالب متل '-1001234567890'                       ║
// ║                                                                  ║
// ║  ⚠لازم كل شخص يفتح محادثة مع البوت ويبعت /start مرة     ║
// ║   قبل ما تضيف رقمو، وإلا تليجرام بيرجّع chat not found  ║
// ╚══════════════════════════════════════════════════════════════════╝
const RECIPIENTS = [
  '1038608008',              // الرقم الأول
  'حط_الرقم_التاني_هون',      // الرقم التاني — بدّل هالنص بالرقم
];

// أي قيمة مو رقم (متل النص التذكيري فوق) بتنتجاهل لحالها،
// فما بينكسر الإرسال لبقية الأرقام.
...
// 4) رسالة لكل جزء × لكل مستقبِل.
//    منرتّب حسب المستقبِل أولاً حتى كل واحد يوصلو (1/2) ثم (2/2) بالترتيب.
const out = [];
  ...
  output: [
    {
      ...
      part: 1,
      totalParts: 2,
      dateLabel: 'الجمعة، 28 أغسطس 2026',
    },
  ],
});
...
const noteTelegram = sticky(
  '## 💬 الإرسال والمستقبِلين\n\n' +
    '### مين بتوصلو النشرة؟\n' +
    'قائمة `RECIPIENTS` بأول عقدة **Split For Telegram**.\n' +
    'زيد أو شيل رقم من هناك وبس — ما في داعي عقدة إرسال تانية.\n\n' +
    'حقل Chat ID هون صار `{{ $json.chatId }}` وما لازم تلمسو.\n\n' +
    '### لإضافة رقم جديد\n' +
    '1. الشخص يفتح محادثة مع البوت ويبعت `/start` — **إلزامي**، تليجرام بيمنع البوت يبلّش محادثة.\n' +
    '2. يأخذ رقمو من @get_id_bot\n' +
    '3. حط الرقم بقائمة RECIPIENTS بين علامتين مع فاصلة\n\n' +
    'محادثة شخص = رقم عادي `1038608008`\n' +
    'قناة = رقم سالب `-1001234567890` (والبوت لازم يكون Admin فيها)\n\n' +
    '✅ أي قيمة مو رقم بتنتجاهل لحالها، فما بتكسر الإرسال لبقية الأرقام.\n\n' +
    '### التنسيق\n' +
    'النشرة تُرسل بـ parse_mode = HTML. العقدة تهرّب كل الرموز ثم تعيد وسوم b و i و code فقط، وتوازن الوسوم في كل رسالة، لأن أي وسم غير مغلق يجعل تليجرام يرفض الرسالة بالكامل.\n\n' +
    'النص يُقسّم لرسائل بحد 3500 حرف مع ترقيم (1/2)، وكل مستقبِل يأخذ أجزاءه بالترتيب.',
